...
kubectl --context east get nodes >/dev/null 2>&1

if [ $? -ne 0 ]; then \
    echo "The east cluster is not running. Please run ./localcluster create clusters" >&2 ;\
    exit 1 ;\
fi

kubectl --context west get nodes >/dev/null 2>&1

if [ $? -ne 0 ]; then \
    echo "The west cluster is not running. Please run ./localcluster create clusters" >&2 ;\
    exit 1 ;\
fi
